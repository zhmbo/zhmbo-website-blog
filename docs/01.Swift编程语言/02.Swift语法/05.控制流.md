---
title: 《Swift编程语言》- 控制流
date: 2020-11-10 16:01:28
permalink: /pages/544cb0/
categories:
  - Swift编程语言
tags:
  - 
...
字典的内容理论上是无序的，遍历元素时的顺序是无法确定的。将元素插入字典的顺序并不会决定它们被遍历的顺序。关于数组和字典的细节，参见 [集合类型](/pages/4b4c78/)。
...
在某些情况下，你可能不想使用包括两个端点的闭区间。想象一下，你在一个手表上绘制分钟的刻度线。总共 `60` 个刻度，从 `0` 分开始。使用半开区间运算符（`..<`）来表示一个左闭右开的区间。有关区间的更多信息，请参阅 [区间运算符](/pages/279dd7/#区间运算符（range-operators）)。
...
![img](https://cdn.jsdelivr.net/gh/itzhangbao/supplies/img/20201110183103.png)
...
> 注意
>
> 虽然在 Swift 中 `break` 不是必须的，但你依然可以在 case 分支中的代码执行完毕前使用 `break` 跳出，详情请参见 [Switch 语句中的 break](#switch-语句中的-break)。
...
为了可读性，符合匹配可以写成多行形式，详情请参考 [复合匹配](#复合型-cases)。

> 注意
>
> 如果想要显式贯穿 case 分支，请使用 `fallthrough` 语句，详情请参考 [贯穿](#贯穿（fallthrough）)。
...
![img](https://cdn.jsdelivr.net/gh/itzhangbao/supplies/img/20201110183121.png)
...
![img](https://cdn.jsdelivr.net/gh/itzhangbao/supplies/img/20201110183128.png)
...
![img](https://cdn.jsdelivr.net/gh/itzhangbao/supplies/img/20201110183138.png)
...
我们将会在下面讨论 `continue`、`break` 和 `fallthrough` 语句。`return` 语句将会在 [函数](/pages/b69d99/) 章节讨论，`throw` 语句会在 [错误处理](/pages/cc5b2d/) 章节讨论。
...
![img](https://cdn.jsdelivr.net/gh/itzhangbao/supplies/img/20201110183152.png)
...
在它一般的形式中，可用性条件使用了一个平台名字和版本的列表。平台名字可以是 `iOS`，`macOS`，`watchOS` 和 `tvOS`——请访问 [声明特性](/pages/629202/#声明特性) 来获取完整列表。除了指定像 iOS 8 或 macOS 10.10 的大版本号，也可以指定像 iOS 11.2.6 以及 macOS 10.13.3 的小版本号。

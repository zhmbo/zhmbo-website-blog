...
更多信息请参见 [属性](/pages/ce0459/)、[方法](/pages/efc653/)、[下标](/pages/e35887/)、[构造过程](/pages/1b072a/)、[扩展](/pages/297d1c/) 和 [协议](/pages/82f958/)。
...
更多信息请参见 [继承](/pages/09b0f4/)、[类型转换](/pages/757c02/)、[析构过程](/pages/c40911/) 和 [自动引用计数](/pages/51a2dd/)。
...
结构体和类都使用构造器语法来创建新的实例。构造器语法的最简单形式是在结构体或者类的类型名称后跟随一对空括号，如 `Resolution()` 或 `VideoMode()`。通过这种方式所创建的类或者结构体实例，其属性均会被初始化为默认值。[构造过程](/pages/1b072a/) 章节会对类和结构体的初始化进行更详细的讨论。
...
与结构体不同，类实例没有默认的成员逐一构造器。[构造过程](/pages/1b072a/) 章节会对构造器进行更详细的讨论。
...
![img](https://cdn.jsdelivr.net/gh/zhmbo/static@master/img/20201110184415.png)
...
![img](https://cdn.jsdelivr.net/gh/zhmbo/static@master/img/20201110184444.png)
...
当在定义你的自定义结构体和类的时候，你有义务来决定判定两个实例“相等”的标准。在章节 [等价运算符](/pages/9854b9/#等价运算符) 中将会详细介绍实现自定义 == 和 != 运算符的流程。

...
正如你将会在 [简化 Getter 声明](/pages/ce0459/#简化-getter-声明) 里看到的， 一个属性的 getter 也可以使用隐式返回的形式。
...
定义一个输入输出参数时，在参数定义前加 `inout` 关键字。一个 `输入输出参数`有传入函数的值，这个值被函数修改，然后被传出函数，替换原来的值。想获取更多的关于输入输出参数的细节和相关的编译器优化，请查看 [输入输出参数](#输入输出参数) 一节。

...
![img](https://cdn.jsdelivr.net/gh/zhmbo/static@master/img/20201110184804.png)
...
`SmallNumber` 的定义包括三个构造器——`init()`、`init(wrappedValue:)` 和 `init(wrappedValue:maximum:)`——下面的示例使用这三个构造器来设置被包装值和最大值。有关构造过程和构造器语法的更多信息，请参考 [构造过程](/pages/1b072a/)。
...
![img](https://cdn.jsdelivr.net/gh/zhmbo/static@master/img/20201110184751.png)
